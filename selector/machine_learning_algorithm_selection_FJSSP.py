...
data = pd.read_csv('instance_features_FJSSP_30s.csv', sep=';')
...
data["Total_job_mean"]=[float(str(i).replace(",", ".")) for i in data["Total_job_mean"]]
...
X = data[columns_list[0:11]] # Input data
...
dt_model_FJSSP = DecisionTreeClassifier(max_depth= 100)

#Train Decision Tree Classifier
dt_model_FJSSP = dt_model_FJSSP.fit(X_train, y_train)

#Predict the response for test dataset
y_pred_dt = dt_model_FJSSP.predict(X_test)
...
rf_model_FJSSP = RandomForestClassifier(n_estimators=100)

rf_model_FJSSP.fit(X_train, y_train)
y_pred_rf = rf_model_FJSSP.predict(X_test)
...
list_neigh = [3, 5, 7, 11]
for n in list_neigh:
  knn_model_FJSSP = KNeighborsClassifier(n_neighbors=n)
  knn_model_FJSSP.fit(X_train, y_train)
  knn_predictions = knn_model_FJSSP.predict(X_test)
...
#calling and training the model in one line
mlp_model_FJSSP = MLPClassifier(random_state=43, hidden_layer_sizes=(50,60),
                          activation='logistic',
                          max_iter=1000).fit(X_train, y_train)

#activation {'identity', 'logistic', 'tanh', 'relu'}, default='relu'

#The above line can be done as follows:
#mlp_model = MLPClassifier(random_state=1, max_iter=300)
#mlp_model.fit(X_train, y_train)

predictions = mlp_model_FJSSP.predict(X_test)
...
'''def algorithm_selection_new_instance(file_analysed):
  ...
  algorithm = dt_model_FJSSP.predict(instance_new)
  algorithm2 = rf_model_FJSSP.predict(instance_new)
  algorithm3 = knn_model_FJSSP.predict(instance_new)
  algorithm4 = mlp_model_FJSSP.predict(instance_new)
